...
public:
	...
	{
		Position = YVec3f((MWorld::MAT_SIZE_METERS) / 2, (MWorld::MAT_SIZE_METERS) / 2, MWorld::MAT_HEIGHT_METERS + 1);
		Height = 1.0f;
	...
	void DrawPickUp() {
		...
		glVertex3d(Cam->LookAt.X, Cam->LookAt.Y, Cam->LookAt.Z);
		glEnd();
	}
	...
	void update(float elapsed)
	{
		...
		CameraPosition = Position + Cam->Direction;

		//Par defaut, on applique la gravité (-100 sur Z), la moitie si dans l'eau
		YVec3f force = YVec3f(0, 0, -1) * 9.81f * 10;
		if (InWater)
		...
		if (Jump)
		{
			force += YVec3f(0, 0, 1) * 50.0f / elapsed; //(impulsion, pas fonction du temps)
			Jump = false;
		}
		...
		if (Cam->ControllerType == Cam->FIRST_PERSON_CONTROLLER)
		{
			Cam->moveTo(CameraPosition);
		}

		if (Cam->ControllerType == Cam->THIRD_PERSON_CONTROLLER)
		{
			Cam->setPosition(Position - (Cam->Direction * ZoomDistance));
